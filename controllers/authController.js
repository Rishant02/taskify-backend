...
module.exports.registerUser = async (req, res, next) => {
  try {
    const { username, email, password, passwordConfirm, name, dept, emp_code } =
      req.body;
    ...
    verificationTokenExpiresAt.setHours(
      verificationTokenExpiresAt.getHours() + 3
    );
...
module.exports.verifyEmail = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .send({ error: "Invalid verification token. Please sign up first." });
    }
    if (user.verificationTokenExpiresAt < new Date()) {
      user.verificationToken = generateToken();
      user.verificationTokenExpiresAt = new Date();
      user.verificationTokenExpiresAt.setHours(
        user.verificationTokenExpiresAt.getHours() + 3
      );
      await user.save();
      sendMail(verifyEmailOption(user));
      return res.send(
        "Verification token expired. A new verification token has been sent to your email address."
      );
    }
    ...
    return res
      .status(200)
      .send({ message: `${user.email} has been verified. You may login now.` });
  } catch (error) {
    next(error);
  }
};
...
module.exports.loginUser = async (req, res, next) => {
  try {
    ...
    const oldUser = validator.isEmail(input)
      ? await User.findOne({ email: input }).populate("tasks")
      : await User.findOne({ emp_code: input }).populate("tasks");
    if (!oldUser) {
      throw new Error(
        `${input} does not exists. Try again or reset your password.`
      );
    }
    if (!oldUser.isVerified) {
      throw new Error("Please verify your email address first.");
    }
    const match = oldUser.passwordChanged
      ? bcrypt.compareSync(password, oldUser.password)
      : password === oldUser.password;
    if (!match) {
      throw new Error(
        "Wrong username or password. Please try again or reset password."
      );
    }
...
module.exports.forgotPasswordReq = async (req, res, next) => {
  try {
    ...
    user.passwordResetToken = otplib.authenticator.generate(
      user._id.toString()
    );
    user.passwordResetTokenExpiresAt = new Date();
    user.passwordResetTokenExpiresAt.setMinutes(
      user.passwordResetTokenExpiresAt.getMinutes() + 30
    );
    sendMail(resetPasswordEmailOption(user));
    await user.save();
    return res.status(200).json({
      passwordToken: user.uniquePasswordToken,
      message: `6 digit OTP has been sent to your email ${user.email}. OTP will expire in 30 minutes.`,
    });
  } catch (err) {
    next(err);
  }
...
module.exports.checkPasswordOTP = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ error: "Invalid reset token. Please sign up first." });
    }
    if (!user.isVerified) {
      return res
        .status(401)
        .json({ error: "User is not verified. Please verify email first" });
    }
    if (user.passwordResetTokenExpiresAt < new Date()) {
      ...
      return res.status(401).json({
        error: "OTP is expired. Please send forgot password request again",
      });
    }
    if (user.passwordResetToken == otp) {
      ...
      await user.save();
      return res.status(200).json({
        isOTPVerified: user.isPasswordOTPVerified,
        message: "OTP is verified. Please change your password.",
      });
    }
    return res.status(401).json({
      error: "Wrong OTP! Please try again or send another reset request",
    });
  } catch (err) {
    next(err);
  }
...
module.exports.resetPassword = async (req, res, next) => {
  try {
    ...
    if (user.passwordResetTokenExpiresAt < new Date()) {
      ...
      return res.status(401).json({
        error:
          "Reset Token is expired. Please send forgot password request again",
      });
    }
...
module.exports.firstLogin = async (req, res, next) => {
  try {
    ...
    if (!user.passwordChanged) {
      ...
      return res
        .status(403)
        .json({ error: "You have already updated your password" });
    }
  } catch (err) {
    next(err);
  }
