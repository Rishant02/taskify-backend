...
module.exports.getAllTasks = async (req, res, next) => {
  try {
    ...
    if (byLoggedUser) {
      ...
      const userIds = await Users.find({ dept: loggedUser?.dept }).select(
        "_id"
      );
      tasks = await Tasks.find({
        $or: [
          ...
          {
            $and: [{ author: req.userID }, { assignTo: req.userID }],
          },
        ],
      })
        .populate("author", "-password")
    ...
    const pipeline = [
      ...
      },
      {
        $group: {
          ...
          numOwnedTasks: {
            $sum: {
              $cond: [{ $eq: [{ $toString: "$author" }, req.userID] }, 1, 0],
            },
          },
          numOpenedTasks: {
    ...
    tasks.forEach((task) => {
      ...
      const unreadNotification = task.taskUpdates.reduce((unread, update) => {
        const data = notifys.filter(
          (notify) =>
            notify.taskUpdate.equals(update._id) && notify.read === false
        );
...
module.exports.createTask = async (req, res, next) => {
  try {
    const { tname, tdesc, status, startDate, dueDate, assignTo, recurAmount } =
      req.body;
  ...
  try {
    ...
    if (["CANCELLED", "ON HOLD"].includes(task?.status)) {
      return res.status(400).json({
        message:
          "You can not mark task complete if status is on hold or cancelled",
      });
    }
    if (task.completed) {
